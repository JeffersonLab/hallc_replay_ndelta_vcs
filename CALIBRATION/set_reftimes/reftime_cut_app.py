...
t_coin_trig_tdcrefcut = {trig_tdcrefcut:.0f}
t_coin_trig_adcrefcut = {trig_adcrefcut:.0f}
...
hdc_tdcrefcut={hdc:.0f}
; cut variable = hT2
hhodo_tdcrefcut={hhodo_tdc:.0f}
; cut variable = hFADC_TREF_ROC1
hhodo_adcrefcut={hhodo_adc:.0f}
hcer_adcrefcut={hcer_adc:.0f}
hcal_adcrefcut={hcal_adc:.0f}
""")
...
pdc_tdcrefcut={pdc:.0f}
; cut variable = pT1
phodo_tdcrefcut={phodo_tdc:.0f}
; cut variable = pFADC_TREF_ROC2
phodo_adcrefcut={phodo_adc:.0f}
pngcer_adcrefcut={pngcer_adc:.0f}
phgcer_adcrefcut={phgcer_adc:.0f}
paero_adcrefcut={paero_adc:.0f}
pcal_adcrefcut={pcal_adc:.0f}
""")
...
class ReftimeCutApp:
    ...
    def get_reference_values(self, ref_dict, name, needed):
            ...
            # Pull the base scalar limit from the spectrometer files (e.g., hhodo_tdcrefcut)
            ...
            if win is not None and win != (0.0, 100000.0):
                ...
                # 0.0 in tcoin.param usually means unassigned/defaulted
                if win_lo > 0.0:
                    if ref_lo is not None:
                        # Choose the tighter, more conservative (higher) lower limit
                        ref_lo = max(ref_lo, win_lo)
                    else:
                        ref_lo = win_lo
    ...
    def draw_channel(self):
        ...
        if ref_lo is not None or ref_hi is not None:
            ...
            ref_lo_str = f"{ref_lo:.0f}" if ref_lo is not None else "-"
            ref_hi_str = f"{ref_hi:.0f}" if ref_hi is not None else "-"
    ...
    def redraw_clicks(self):
        ...
        if len(self.clicks) > 0:
            ...
            lo_str = f"{lo:.0f}"
            hi_str = f"{hi:.0f}" if hi is not None else ("(n/a)" if kind == "adc" else "-")
    ...
    def update_info_panel(self):
        ...
        for n in NEEDED_CHANNELS:
            ...
            if r is not None and r.lo is not None:
                ...
                lines.append(f"{mark} {n}: {r.lo:.0f}{tag}")
            else:
